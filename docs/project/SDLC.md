...
4. **PR** — a pull request is opened against the branch's target per Gitflow, and is under review. The story moves to `docs/stories/4. PR/` and `pr_opened` is stamped (optionally also filling in `pr_url`). Opening, reviewing, and merging a PR is always a manual, explicitly-confirmed human action — nothing in this SDLC or its tooling auto-opens or auto-merges a PR.
